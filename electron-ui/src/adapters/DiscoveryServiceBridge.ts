...
import type { DiscoveryService, PeerIdentity } from '@connection/core'
import type { DiscoveryServiceAdapter, DiscoveredDevice, DeviceIdentityCredential } from '@device/core'
...
export class DiscoveryServiceBridge implements DiscoveryServiceAdapter {
  ...
  private setupEventListeners(): void {
    // Bridge peerDiscovered -> onDeviceDiscovered
    this.discoveryService.on('peerDiscovered', (peer: PeerIdentity) => {
      const device = this.convertPeerToDevice(peer)
      this.deviceDiscoveredCallbacks.forEach(callback => callback(device))
    })

    // Bridge peerLost -> onDeviceLost
    this.discoveryService.on('peerLost', (peer: PeerIdentity) => {
