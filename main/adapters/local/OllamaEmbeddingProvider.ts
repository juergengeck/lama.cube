 ...
 * Provides lifecycle management for consistency with ONNX providers.
 */
...
export class OllamaEmbeddingProvider implements LocalEmbeddingProvider {
  ...
  constructor(
    ollamaModelName: string = 'nomic-embed-text',
    baseUrl?: string
  ) {
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      const hasModel = models.some(m =>
        m.name === this.ollamaModelName ||
        m.name.startsWith(`${this.ollamaModelName}:`)
      );

      if (!hasModel) {
        console.warn(
          `[OllamaEmbedding] Model '${this.ollamaModelName}' not found. ` +
          `Available: ${models.map(m => m.name).join(', ')}. ` +
          `Pull with: ollama pull ${this.ollamaModelName}`
        );
