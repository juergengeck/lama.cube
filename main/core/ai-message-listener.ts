...
class AIMessageListener {
  ...
  async scanForUnansweredMessages(ownerId: string | undefined): Promise<void> {
    ...
    for (const topicId of aiTopics) {
      try {
        ...
        // Enter the topic room (topicId is stored as string but is actually SHA256IdHash)
        const topicRoom = await topicModel.enterTopicRoom(topicId as SHA256IdHash<Topic>)
        if (!topicRoom) {
  ...
  async registerAITopic(topicId: string, aiPersonId: SHA256IdHash<Person>): Promise<void> {
    ...
    if (this.aiAssistantModel) {
      await this.aiAssistantModel.registerAITopic(topicId, aiPersonId)
    } else {
   ...
   */
  async handleChannelUpdate(channelInfoIdHash: string, updateInfo: any): Promise<void> {
    console.log(`[AIMessageListener] Processing channel update for ${channelInfoIdHash}`)
    ...
    try {
      ...
      let topic = allTopics.find((t: Topic) => t.channel === channelInfoIdHash)

      // If no exact match, look up by discriminator (group chat with multiple channels)
      if (!topic) {
        try {
          // Get incoming channel's discriminator
          const incomingChannelResult = await getObjectByIdHash(channelInfoIdHash as any)
      ...
      // Calculate actual topicIdHash for all operations
      ...
      // Enter the topic room to get messages using actual topicIdHash
      const topicRoom = await topicModel.enterTopicRoom(topicIdHash as SHA256IdHash<Topic>)
      if (!topicRoom) {
      ...
      if ((messages as any)?.length > 0) {
        ...
        console.log(`[AIMessageListener] 💬 Processing message in topic ${topicDisplayName}: "${messageText.substring(0, 50)}..." (has AI participant)`)

        // Delegate to AIAssistantModel to process the message and generate response using actual topicIdHash
        try {
          await this.aiAssistantModel.processMessage(topicIdHash, messageText, messageSender)
          console.log(`[AIMessageListener] ✅ LLM response generated for topic ${topicDisplayName}`)
        } catch (error) {
