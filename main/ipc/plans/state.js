/**
 * State Management IPC Plans (TypeScript)
 */
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var manager_js_1 = require("../../state/manager.js");
var statePlans = {
    getState: function (event_1) {
        return __awaiter(this, arguments, void 0, function (event, request) {
            var path;
            if (request === void 0) { request = {}; }
            return __generator(this, function (_a) {
                path = request.path;
                console.log('[StateHandler] Get state request:', path || 'all');
                if (path) {
                    return [2 /*return*/, manager_js_1.default.getState(path)];
                }
                // Return full serializable state
                return [2 /*return*/, manager_js_1.default.toJSON()];
            });
        });
    },
    setState: function (event, request) {
        return __awaiter(this, void 0, void 0, function () {
            var key, value;
            return __generator(this, function (_a) {
                key = request.key, value = request.value;
                console.log('[StateHandler] Set state request:', key, '=', value);
                try {
                    manager_js_1.default.setState(key, value);
                    return [2 /*return*/, { success: true, key: key, value: value }];
                }
                catch (error) {
                    console.error('[StateHandler] Error setting state:', error);
                    return [2 /*return*/, { success: false, error: error.message }];
                }
                return [2 /*return*/];
            });
        });
    },
    subscribe: function (event_1) {
        return __awaiter(this, arguments, void 0, function (event, request) {
            var _a, paths, unsubscribers, handler_1;
            if (request === void 0) { request = {}; }
            return __generator(this, function (_b) {
                _a = request.paths, paths = _a === void 0 ? [] : _a;
                console.log('[StateHandler] Subscribe to state changes:', paths);
                unsubscribers = [];
                // Subscribe to specific paths or all changes
                if (paths.length === 0) {
                    handler_1 = function (change) {
                        event.sender.send('state:changed', change);
                    };
                    manager_js_1.default.on('stateChanged', handler_1);
                    // Store unsubscriber
                    unsubscribers.push(function () {
                        manager_js_1.default.off('stateChanged', handler_1);
                    });
                }
                else {
                    // Subscribe to specific paths
                    paths.forEach(function (path) {
                        var unwatch = manager_js_1.default.watch(path, function (newValue, oldValue) {
                            event.sender.send('state:changed', {
                                path: path,
                                newValue: newValue,
                                oldValue: oldValue
                            });
                        });
                        unsubscribers.push(unwatch);
                    });
                }
                // Handle cleanup when renderer disconnects
                event.sender.once('destroyed', function () {
                    unsubscribers.forEach(function (unsub) { return unsub(); });
                });
                return [2 /*return*/, { subscribed: true, paths: paths }];
            });
        });
    }
};
export default statePlans;
