 ...
 * IPC system.
 *
 * Uses centralized event registry from @refinio/lama.core/events for type-safe event names.
 */
...
import localModelsPlans, { chatWithLocalDirect } from '../ipc/plans/local-models.js';
...
export class ElectronLLMPlatform implements LLMPlatform {
  ...
  async getInstalledTextGenModels(): Promise<Array<{
    ...
    // Dynamic import: @refinio/local.core is optional and may not be installed
    const { getTextGenerationModels } = await import('@refinio/local.core');
