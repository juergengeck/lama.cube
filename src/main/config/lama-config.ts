...
/**
 * Get default export directories for memories.
 * These directories are outside the app data directory and survive "Delete all App Data".
 */
function getDefaultExportDirectories(): string[] {
    const homeDir = os.homedir();
    const platform = process.platform;
    const directories: string[] = [];

    // Primary: Documents folder (all platforms)
    if (platform === 'win32') {
        directories.push(path.join(homeDir, 'Documents', 'LAMA', 'memories'));
    } else {
        directories.push(path.join(homeDir, 'Documents', 'LAMA', 'memories'));
    }

    // macOS: iCloud Documents (if likely available)
    if (platform === 'darwin') {
        directories.push(path.join(homeDir, 'Library', 'Mobile Documents', 'com~apple~CloudDocs', 'LAMA', 'memories'));
    }
...
export interface LamaConfig {
    instance: {
        ...
        /** Directories for auto-exporting memories (outside app data, survives deletion) */
...
const defaultConfig: LamaConfig = {
    instance: {
        ...
        // Export directories outside app data (survive "Delete all App Data")
        exportDirectories: getDefaultExportDirectories(),
        wipeStorage: false
    },
    network: {
        commServer: {
...
function parseCLIArgs(): Partial<LamaConfig> {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        } else if (arg.startsWith('--export-directories=')) {
            if (!config.instance) config.instance = {};
            // Comma-separated list of directories
            config.instance.exportDirectories = arg.split('=')[1].split(',').map(d => d.trim());
...
export async function loadConfig(): Promise<LamaConfig> {
    ...
    if (process.env.LAMA_EXPORT_DIRECTORIES) {
        // Comma-separated list of directories
        config.instance.exportDirectories = process.env.LAMA_EXPORT_DIRECTORIES.split(',').map(d => d.trim());
    }
    ...
    if (config.commServer?.url) {
        console.log('[LamaConfig] Migrating legacy commServer config to network.commServer');
        config.network.commServer.url = config.commServer.url;
        delete config.commServer;
    }
    ...
    console.log('[LamaConfig] Final configuration:', {
        ...
        exportDirectories: config.instance.exportDirectories,
