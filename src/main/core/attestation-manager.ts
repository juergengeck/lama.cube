...
export class AttestationManager {
  ...
  async createAttestation(params: any): Promise<any> {
    ...
    try {
      ...
      const auditorId = await me.mainIdentity();
      const auditorName = (profile as any)?.name || 'Unknown Auditor'; // Profile name accessor varies
      ...
      // Custom object type - cast needed for storeUnversionedObject
      const result = await storeUnversionedObject(attestation as any); // Custom MessageAttestation type
  ...
  async storeInTopic(topicId: any, attestation: any, attestationHash: any): Promise<any> {
    ...
    try {
      ...
      // ChannelManager method not in standard type definitions
      await (this.channelManager as any).addToChannel(topicId, channelEntry); // Custom ChannelManager method
  ...
  async getAttestationsForMessage(messageHash: any): Promise<any> {
    ...
    try {
      ...
      for (const hash of attestationHashes) {
        try {
          const attestation = await getObject(hash) as unknown as MessageAttestation;
  ...
  async getAttestationsForTopic(topicId: any): Promise<any> {
    ...
    try {
      // Get all channel entries for the topic - custom ChannelManager method
      const entries = await (this.channelManager as any).getChannelEntries(topicId); // Custom method
  ...
  async verifyAttestation(attestationHash: any, expectedMessageHash: any): Promise<any> {
    try {
      ...
      const attestation = await getObject(attestationHash) as unknown as MessageAttestation & { signature?: any };
  ...
  async syncAttestations(topicId: any): Promise<any> {
    ...
    try {
      ...
      await (this.channelManager as any).syncChannel(topicId); // Custom ChannelManager method
