...
export async function grantMutualChannelAccess(participantsHash: SHA256Hash<HashGroup<Person>>, person1Id: SHA256IdHash<Person>, person2Id: SHA256IdHash<Person>, discriminator?: string): Promise<boolean> {
  try {
    ...
    // Grant INDIVIDUAL access to each person (required for CHUM sync)
    ...
    console.log('[ChannelAccess] ✅ Individual access established for both persons')
    return true
  } catch (error) {
...
export async function setupBrowserNodeChannelAccess(nodeOwnerId: SHA256IdHash<Person>, browserPersonId: SHA256IdHash<Person>, channelManager: any): Promise<boolean> {
  try {
    ...
    for (const channelInfo of channelInfos) {
      ...
      // Only grant access to channels where node owner is a participant
      // Browser and node are the same person (IoM), so browser gets access to channels
      // the person participates in - including AI topics where user + AI are participants
      try {
        // Get the HashGroup to check participants
        const participantsGroup = await getObject(channelInfo.participants) as HashGroup<Person>
        const participantIds = participantsGroup?.person ? Array.from(participantsGroup.person) : []

        // Check if node owner is a participant in this channel
        const isParticipant = participantIds.some(p => p === nodeOwnerId)

        if (!isParticipant) {
          continue // Skip channels where user is not a participant
        }
        ...
        // Log AI channel access grants for debugging
        if (channelOwner && channelOwner !== nodeOwnerId) {
          console.log(`[ChannelAccess] ✅ Granted browser access to AI channel (owner: ${channelOwner?.substring(0, 8)}, participants: ${participantIds.length})`)
        }
      } catch (error) {
        console.warn(`[ChannelAccess] Could not check participants for channel:`, (error as Error).message)
      }
    }
