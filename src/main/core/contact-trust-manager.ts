...
interface AcceptanceOptions {
  ...
  notes?: string;
  canMessage?: boolean;
  canShareChannels?: boolean;
  canCall?: boolean;
  canShareFiles?: boolean;
  canSeePresence?: boolean;
  discoveryVCId?: string;
}
...
class ContactTrustManager extends EventEmitter {
   ...
   */
  async createAcceptanceVC(personId: any, options: AcceptanceOptions = {}): Promise<unknown> {
    const instanceId = getInstanceIdHash()
    const myPersonId = getInstanceOwnerIdHash()
    if (!myPersonId) {
      throw new Error('No person ID available')
  ...
  async storeContactWithVC(credential: any, discoveryVC: any): Promise<any> {
    ...
    const personKeysResult = await storeUnversionedObject(personKeysObj)
    ...
    const personResult = await storeVersionedObject(person as any) // Custom Person object
    const personIdHash = personResult.idHash
