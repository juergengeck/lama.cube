...
import { AffirmationLicense } from '@refinio/one.models/lib/recipes/Certificates/AffirmationCertificate.js';
...
export class MessageAssertionManager {
  ...
  async verifyMessageAssertion(certificateHash: any, expectedMessageHash: any): Promise<any> {
    try {
      ...
      const assertionData = assertionDataResult.obj as any; // ONE.core generic return → MessageAssertion
