  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (keys.length > 0) {
        ...
        const certResult = await trust.certify('TrustKeysCertificate', { profile: profile.loadedVersion })
        tracePairing('trust_cert_issued', { profileHash: profile.loadedVersion })

        // Share the certificate and signature with the remote peer
        // This enables mutual trust verification
        try {
          // Share all certificate-related objects with the remote peer
          await createAccess([
            {
          ...
          tracePairing('trust_cert_shared', { to: remotePersonId })
        } catch (shareError) {
          tracePairing('trust_cert_share_failed', { error: (shareError as Error).message })
          // Non-fatal - local trust is still established
        }
