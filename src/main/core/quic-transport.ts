...
import type { QuicVCPeerTrustInfo as PeerTrustInfo, QuicVCCredential as DeviceIdentityCredential } from '@refinio/connection.core'
...
export default class QuicTransport extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    if (this.nodeOneCore.ownPersonId) {
      ...
      // Initialize QuicVC manager with transport (credential set later via setTrustVerifier)
      // Note: ownCredential must be set before connections can be established
      // This is typically done by the discovery system which knows our public key

      // Set up event forwarding from QuicVC manager using OEvent
      this.quicvcManager.onConnectionEstablished.listen((info) => {
        this.emit('deviceConnected', info)
      })

      this.quicvcManager.onConnectionClosed.listen((deviceId, reason) => {
