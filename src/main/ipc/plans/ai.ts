...
import { AICreationService, type CreationContext } from '@refinio/lama.core/services/AICreateService.js';
import { CreationContextCollector, type CreationContextProvider } from '@refinio/lama.core/services/CreateContextCollector.js';
import createUserSettingsPlans from './user-settings.js';
import { storeVersionedObject } from '@refinio/one.core/lib/storage-versioned-objects.js';
const { BrowserWindow } = electron;

/**
 * Emit AI creation progress to renderer
 */
function emitAICreationProgress(step: number, total: number, message: string, name: string = '') {
  const mainWindow = BrowserWindow.getAllWindows()[0];
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('ai:creation-progress', { step, total, message, aiId: '', name });
  }
}
...
const aiPlans = {
  ...
  async chat(
    ...
    try {
      ...
      if (modelId.startsWith('claude:')) {
        try {
          const handlers = createUserSettingsPlans(nodeOneCore);
  ...
  'ai:getDefaultModel': async (event: IpcMainInvokeEvent): Promise<string | null> => {
    try {
      ...
      // Get from AIAssistantHandler which loads from SettingsPlan
      ...
      // Fallback: Read directly from SettingsPlan if aiAssistantModel not available
      if (!modelId && nodeOneCore.settingsPlan) {
        const response = await nodeOneCore.settingsPlan.getSection({ moduleId: 'ai' });
        if (response.values?.defaultModelId) {
          console.log('[AI IPC] getDefaultModel fallback - found modelId:', response.values.defaultModelId);
          modelId = response.values.defaultModelId;
        }
      }
      ...
      if (modelId) {
        ...
        // CRITICAL: Also verify an AI Person exists
        // After "delete all data", settings may have stale modelId but no AI Person
        if (nodeOneCore.aiAssistantModel?.getAIManager) {
          const allAIs = nodeOneCore.aiAssistantModel.getAIManager().getAllAIs();
          if (!allAIs || allAIs.length === 0) {
            console.log(`[AI IPC] getDefaultModel: no AI Persons exist, returning null`);
            return null;
          }
        }
      }
  ...
  async switchAIModel(
    ...
    try {
      const handler = getAIHandler();
      await handler.switchAIModel(aiPersonId as SHA256IdHash<Person>, modelId);
  ...
  /**
   * Get active stream state for a topic (used to restore UI on navigation)
   * Returns the current accumulated content if streaming is in progress
   */
  async getActiveStream(
    ...
    try {
      const handler = getAIHandler();
      const messageProcessor = handler.getMessageProcessor();
      const activeStream = messageProcessor.getActiveStream(topicId);
      return { success: true, data: activeStream };
    } catch (error: any) {
      console.error('[AI IPC] getActiveStream error:', error);
      return { success: false, error: error.message };
    }
  },
  ...
  async setAISettings(
    ...
    try {
      ...
      let aiSettings = topic.aiParticipants.get(aiPersonId as SHA256IdHash<Person>);
      if (!aiSettings) {
        aiSettings = {
      ...
      topic.aiParticipants.set(aiPersonId as SHA256IdHash<Person>, aiSettings);
  ...
  async generateAIName(
    ...
    console.log(`[AI IPC] ⏱️ generateAIName START - model: ${modelId}`);

    // Progress: Step 1 - Connecting to model
    emitAICreationProgress(1, 5, 'Connecting to model...');
    ...
    try {
      // Collect context (device, locale, time) using OS hostname (not ONE.core instance name)
      // Instance name is "LAMA Instance" which pollutes the prompt and causes LLM to echo "Lama"
      const t1 = performance.now();
      const contextProvider: CreationContextProvider = {
        async getDeviceName() {
      ...
      console.log(`[AI IPC] ⏱️ Context collected: ${(performance.now() - t1).toFixed(0)}ms`);
      ...
      const creationService = new AICreationService(async (messages, reqModelId) => {
        ...
        // Progress: Step 2 - Model is thinking (this is the long wait)
        emitAICreationProgress(2, 5, 'Model is thinking...');

        if (isOnDeviceModel) {
          // On-device models: bypass storage (local inference)
          // stream: false - AI name generation only needs final result
          response = await llmManager.chatLocalDirect(reqModelId, messages, { disableTools: true, stream: false });
        } else {
          ...
          // stream: false - AI name generation only needs final result
          const result = await llmManager.chat(messages, reqModelId, { disableTools: true, stream: false });
          if (typeof result === 'string') {
            response = result;
          } else if (result && typeof result === 'object' && 'content' in result) {
            response = (result as { content: string }).content || '';
          } else {
            response = JSON.stringify(result);
          }
        }

        console.log(`[AI IPC] ⏱️ LLM inference: ${(performance.now() - tLLM).toFixed(0)}ms`);
        return response;
      });
      ...
      console.log(`[AI IPC] ⏱️ creationService.generateName: ${(performance.now() - t2).toFixed(0)}ms`);

      // Progress: Update with generated name
      emitAICreationProgress(2, 5, 'Name generated!', result.name);

      console.log(`[AI IPC] ⏱️ generateAIName TOTAL: ${(performance.now() - t0).toFixed(0)}ms - name: ${result.name}`);
