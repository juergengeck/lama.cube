...
import { AuditPlan } from '@refinio/lama.core/plans/AuditPlan.js';
import type { TrustManager } from '../../types/extended-types.js';
 ...
 */
export function resetAuditPlanSingletons(): void {}
...
function getAttestationManager(): AttestationManager | null {
  if (!attestationManager && nodeOneCore.initialized) {
    attestationManager = new AttestationManager(
      ...
      (nodeOneCore.trustPlan ?? null) as TrustManager | null,
...
function getHandler(): AuditPlan {
  if (!auditHandler || auditEpoch !== nodeOneCore.initEpoch) {
    ...
    auditEpoch = nodeOneCore.initEpoch;
  }
  return auditHandler;
}
