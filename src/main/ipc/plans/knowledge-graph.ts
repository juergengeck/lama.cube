/**
 * IPC plans for knowledge graph data
...
import { cosineSimilarity } from '@refinio/meaning.core';
...
interface GraphNode {
  ...
  parent?: string;
  children?: string[];
  // Embedding for semantic positioning (768 dims, nomic-embed-text-v1.5)
  embedding?: number[];
  metadata?: {
    createdAt?: number;
    messageCount?: number;
...
export default function registerKnowledgeGraphPlans(handle: (channel: string, handler: any) => void, nodeOneCore: any) {
  /**
   * Get knowledge graph data (nodes + edges)
   */
  handle('memory:getKnowledgeGraph', async (event: IpcMainInvokeEvent) => {
    try {
      ...
      // Add Memory nodes from journal topic
      try {
              ...
              if (attachment && attachment.$type$ === 'Assembly') {
                ...
                if (wrappedHash) {
                  ...
                  if (wrapped && wrapped.$type$ === 'Memory') {
                    ...
                    const memoryNodeId = `memory:${attachmentHash}`;

                    nodes.push({
                      id: memoryNodeId,
                      type: 'memory',
                      label: memory.title || 'Untitled Memory',
                      keywords: [], // Memories don't have direct keywords
                      metadata: {
                        createdAt: message.sendTime
                      }
                    });

                    // Track source subject connections for later edge creation
                    if (memory.sourceSubjects) {
                      for (const sourceSubjectId of memory.sourceSubjects) {
                        // We'll create edges after collecting all nodes
                        keywordToNodes.set(`_memory_source:${memoryNodeId}:${sourceSubjectId}`, [memoryNodeId]);
                      }
                    }
                  }
                }
        ...
        console.log(`[KnowledgeGraph] Added ${nodes.filter(n => n.type === 'memory').length} memory nodes`);
      } catch (err) {
        console.warn('[KnowledgeGraph] Failed to scan lama topic for memories:', err);
      }

      // Generate embeddings for subject and memory nodes
      const inferenceManager = getInferenceManager();
      if (inferenceManager.initialized) {
        const embeddableNodes = nodes.filter(n => n.type === 'subject' || n.type === 'memory');
        const textsToEmbed = embeddableNodes.map(n => n.label);

        if (textsToEmbed.length > 0) {
          console.log(`[KnowledgeGraph] Generating embeddings for ${textsToEmbed.length} nodes...`);
          try {
            const embeddings = await inferenceManager.embedBatch(textsToEmbed);

            for (let i = 0; i < embeddableNodes.length; i++) {
              embeddableNodes[i].embedding = embeddings[i];
            }
  ...
  console.log('[IPC] Knowledge graph plans registered');
}
