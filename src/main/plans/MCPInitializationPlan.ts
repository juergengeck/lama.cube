...
import { getInferenceManager } from '@refinio/lama.core/services/inference-manager.js';
...
export class MCPInitializationPlan {
  ...
  private async registerNodeOneCore(nodeOneCore: any): Promise<any> {
    ...
    // Register InferenceManager for image generation tools
    try {
      const inferenceManager = getInferenceManager();
      await mcpManager.setInferenceManager(inferenceManager);
      console.log('[MCPInitializationPlan] ✅ Inference tools registered (image generation)');
    } catch (error) {
      console.warn('[MCPInitializationPlan] Failed to register inference tools:', error);
    }
