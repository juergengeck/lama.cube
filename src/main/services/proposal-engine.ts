...
import type { Subject as CoreSubject } from '@refinio/lama.core/one-ai/types/Subject.js';
import type { Keyword as CoreKeyword } from '@refinio/lama.core/one-ai/types/Keyword.js';
...
/** Minimal interface for memory plan used by ProposalEngine */
interface MemoryPlanLike {
  listSubjects(): Promise<string[]>;
  getSubject(idHash: string): Promise<{ id?: string; name: string; description?: string; created?: number; metadata?: Map<string, unknown> | Record<string, unknown> } | null>;
}

export class ProposalEngine {
  private topicAnalysisModel: unknown;
  private channelManager: unknown;
  private memoryPlan?: MemoryPlanLike;
  private topicManager?: { isLamaTopic?(topicId: string): boolean };

  constructor(topicAnalysisModel: unknown, channelManager: unknown, memoryPlan?: MemoryPlanLike, topicManager?: { isLamaTopic?(topicId: string): boolean }) {
  ...
  private async resolveKeywordTerms(keywordIdHashes: SHA256IdHash<CoreKeyword>[]): Promise<string[]> {
    ...
    for (const idHash of keywordIdHashes || []) {
      try {
        ...
        if (err.code !== 'SB-READ2' || err.type !== 'vheads') {
  ...
  async getMemoryProposals(
    ...
    try {
      ...
      const memoryList = await this.memoryPlan.listSubjects();
      ...
      for (const memoryIdHash of memoryList) {
        try {
          ...
          const memory = await this.memoryPlan.getSubject(memoryIdHash);
