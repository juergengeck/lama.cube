  ...
  async applyConnectionSettings(settings: any): Promise<any> {
    ...
    if (settings.acceptIncomingConnections !== undefined) {
      ...
      if (nodeOneCore.connectionsModel) {
        // ConnectionsModel doesn't expose acceptIncomingConnections as a writable property
      // This is a configuration attempt that may not be supported by the current API
      const cm = nodeOneCore.connectionsModel as ConnectionsModel & { acceptIncomingConnections?: boolean };
      cm.acceptIncomingConnections = settings.acceptIncomingConnections
      }
    }
