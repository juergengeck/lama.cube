...
class StateManager extends EventEmitter {
  ...
  // Message management
  addMessage(topicId: any, message: any): any {
    const messages = new Map(this.state.messages)
    if (!messages.has(topicId)) {
      messages.set(topicId, [])
    }

    const existingMessages = messages.get(topicId)
    if (!Array.isArray(existingMessages)) {
      messages.set(topicId, [message])
    } else {
      const conversationMessages = [...existingMessages, message]
      messages.set(topicId, conversationMessages)
    }
    
    this.setState('messages', messages)
    
    // Also update conversation's last message
    this.updateConversation(topicId, {
      lastMessage: message,
      lastMessageAt: new Date()
    })
  }

  getMessages(topicId: any): any {
    return this.state.messages.get(topicId) || []
  }
