...
export interface LamaAPI {
  ...
  setDefaultModel: (modelId: string, displayName?: string, email?: string) => Promise<boolean>
  switchAIModel: (aiPersonId: string, modelId: string) => Promise<boolean>
  getAIPersonForTopic: (topicId: string) => Promise<string | null>
  setAISettings: (params: { topicId: string; aiPersonId: string; setting: 'analyse' | 'respond' | 'ignore'; enabled: boolean }) => Promise<{ success: boolean; error?: string }>
  ...
  getSubjectResonance: (subject: string) => Promise<{ resonance: number; momentum: 'rising' | 'falling' | 'stable' }>
  attachSubject: (subject: string, hash: string, userId: string, confidence: number, topicId: string) => Promise<void>

  // LLM Identity
  getLLMIdentity: (contactId: string) => Promise<{
    name: string
  ...
  // Jobs
  submitJob: (definition: { type: string; payload: unknown; resumeFrom?: unknown }) => Promise<{ jobId: string }>
  cancelJob: (jobId: string) => Promise<{ success: boolean }>
  listJobs: (filter?: { status?: string[] }) => Promise<any[]>
...
class LamaBridge implements LamaAPI {
  ...
  constructor() {
    ...
    if (window.electronAPI) {
      ...
      // New topic events (from WhatsApp sync)
      window.electronAPI.on('newTopic', (data: any) => {
        console.log('[LamaBridge] 🆕 newTopic received:', data)
        this.emit('newTopic', data)
      })

      // Baileys / WhatsApp events
      window.electronAPI.on(Events.BAILEYS_CONNECTION_CHANGED, createIPCHandler(Events.BAILEYS_CONNECTION_CHANGED))
      window.electronAPI.on(Events.BAILEYS_SYNC_STATS, createIPCHandler(Events.BAILEYS_SYNC_STATS))
      window.electronAPI.on(Events.BAILEYS_IMPORT_PROGRESS, createIPCHandler(Events.BAILEYS_IMPORT_PROGRESS))
      window.electronAPI.on(Events.BAILEYS_MESSAGE_RECEIVED, createIPCHandler(Events.BAILEYS_MESSAGE_RECEIVED))
      window.electronAPI.on(Events.BAILEYS_LOG, createIPCHandler(Events.BAILEYS_LOG))
      window.electronAPI.on(Events.BAILEYS_ERROR, createIPCHandler(Events.BAILEYS_ERROR))
      window.electronAPI.on(Events.BAILEYS_QR_CODE, createIPCHandler(Events.BAILEYS_QR_CODE))
      window.electronAPI.on(Events.BAILEYS_QR_CODE_BASE64, createIPCHandler(Events.BAILEYS_QR_CODE_BASE64))
      window.electronAPI.on(Events.BAILEYS_PAIRING_CODE, createIPCHandler(Events.BAILEYS_PAIRING_CODE))
      window.electronAPI.on(Events.BAILEYS_CHATS_DISCOVERED, createIPCHandler(Events.BAILEYS_CHATS_DISCOVERED))
      ...
      // Job events (worker manager)
      window.electronAPI.on(Events.JOB_PROGRESS, createIPCHandler(Events.JOB_PROGRESS))
      window.electronAPI.on(Events.JOB_STATUS, createIPCHandler(Events.JOB_STATUS))
  ...
  async getMessages(topicId: string): Promise<Message[]> {
    ...
    if (result.messages.length > 0) {
      console.log(`[LamaBridge] Got ${result.messages.length} messages from Node via IPC for ${topicId.substring(0, 20)}`)
    }

    // Transform Node messages to our Message format
    return result.messages.map((msg: any) => ({
      id: msg.id || `msg-${Date.now()}-${Math.random()}`,
      dataHash: msg.dataHash, // Content hash for trace lookup
  ...
  async setDefaultModel(modelId: string, displayName?: string, email?: string): Promise<boolean> {
    if (!window.electronAPI) {
      throw new Error('IPC not available')
    }
    const result = await window.electronAPI.invoke('ai:setDefaultModel', { modelId, displayName, email })
    return result.success
  }
  ...
  async setAISettings(params: {
    topicId: string;
    aiPersonId: string;
    setting: 'analyse' | 'respond' | 'ignore';
    enabled: boolean;
  ...
  async clearConversation(topicId: string = 'default'): Promise<void> {
    if (!window.electronAPI) {
      throw new Error('IPC not available')
    }
    const result = await window.electronAPI.invoke('chat:clearConversation', { topicId })
    if (!result.success) {
      throw new Error(result.error || 'Failed to clear conversation')
    }
  }
  
  async setConversationModel(topicId: string, modelId: string): Promise<void> {
    // Store locally for UI purposes
    await ipcStorage.setItem(`conv-model-${topicId}`, modelId)
  }
  ...
  async getActiveStream(topicId: string): Promise<{ success: boolean; data?: { content: string; modelId: string; modelName?: string } | null }> {
    ...
    const result = await window.electronAPI.invoke('ai:getActiveStream', { topicId })
    return result
  }
  ...
  async getSubjectResonance(subject: string): Promise<{ resonance: number; momentum: 'rising' | 'falling' | 'stable' }> {
    ...
    const result = await window.electronAPI.invoke('topicAnalysis:getSubjectResonance', { subject })
    return result?.data || { resonance: 0.5, momentum: 'stable' }
  }
  ...
  async attachSubject(subject: string, hash: string, userId: string, confidence: number, topicId: string): Promise<void> {
    ...
    await window.electronAPI.invoke('topicAnalysis:attachSubject', { subject, hash, userId, confidence, topicId })
  }
  ...
  async getLLMIdentity(contactId: string): Promise<{
    ...
    const result = await window.electronAPI.invoke('ai:getLLMIdentity', { contactId })
  ...
  async setComposing(topicId: string, isComposing: boolean): Promise<{ success: boolean; error?: string }> {
    ...
    return await window.electronAPI.invoke('chat:setComposing', { topicId, isComposing })
  }
  ...
  // Job methods (worker manager)
  async submitJob(definition: { type: string; payload: unknown; resumeFrom?: unknown }) {
    if (!window.electronAPI) {
      throw new Error('IPC not available')
    }
    return window.electronAPI.invoke('job:submit', definition)
  }
  ...
  async cancelJob(jobId: string) {
    ...
    return window.electronAPI.invoke('job:cancel', jobId)
  }

  async listJobs(filter?: { status?: string[] }) {
    if (!window.electronAPI) {
      throw new Error('IPC not available')
    }
    return window.electronAPI.invoke('job:list', filter)
  }
  ...
  async getJob(jobId: string) {
    ...
    return window.electronAPI.invoke('job:getJob', jobId)
  }
  ...
  async recordFeedback(subjectId: string, feedbackType: 'like' | 'dislike'): Promise<{ success: boolean; error?: string }> {
    ...
    const result = await window.electronAPI.invoke('topics:recordFeedback', {
      subjectId,
      feedbackType
    })
  ...
  async getMyInstances(): Promise<any[]> {
    ...
    const result = await window.electronAPI.invoke('instance:getMyInstances')
    if (!result.success) {
      console.error('[LamaBridge] Failed to get my instances:', result.error)
      return []
    }
    return result.data || []
  }
  ...
  async getContactInstances(): Promise<Record<string, any[]>> {
    ...
    const result = await window.electronAPI.invoke('instance:getContactInstances')
    if (!result.success) {
      console.error('[LamaBridge] Failed to get contact instances:', result.error)
      return {}
    }
    return result.data || {}
  }
  ...
  async getLocalInstance(): Promise<any | null> {
    ...
    const result = await window.electronAPI.invoke('instance:getLocalInstance')
    if (!result.success) {
      console.error('[LamaBridge] Failed to get local instance:', result.error)
      return null
    }
    return result.data || null
  }
  ...
  onInstancesChanged(handler: () => void): () => void {
    ...
    return this.on('connections:changed', handler)
  }
  ...
  async shareToGlue(message: {
    ...
    return await window.electronAPI.invoke('glue:shareToGlue', {
      message,
      ...options
    })
  }
  ...
  async getGlueTopic(): Promise<{ success: boolean; topicId?: string; exists: boolean; error?: string }> {
    ...
    return await window.electronAPI.invoke('glue:getGlueTopic', {})
  }

  // Moltbook integration
  async getMoltConfig(): Promise<{
    success: boolean;
    config: {
      enabled: boolean;
      agentName: string | null;
      profileUrl: string | null;
      submoltUrl: string | null;
      claimed: boolean;
    };
    error?: string;
  }> {
    ...
    return await window.electronAPI.invoke('molt:getConfig', {})
  }

  async setMoltEnabled(enabled: boolean): Promise<{ success: boolean; error?: string }> {
    if (!window.electronAPI) {
      throw new Error('IPC not available')
    }
    return await window.electronAPI.invoke('molt:setEnabled', { enabled })
  }

  async syncMoltNow(): Promise<{ success: boolean; syncedPosts?: number; error?: string }> {
    if (!window.electronAPI) {
      throw new Error('IPC not available')
    }
    return await window.electronAPI.invoke('molt:syncNow', {})
  }

  async getMoltTopic(): Promise<{ success: boolean; topicId?: string; exists: boolean; error?: string }> {
    if (!window.electronAPI) {
      throw new Error('IPC not available')
    }
    return await window.electronAPI.invoke('molt:getMoltTopic', {})
  }

  async getMoltFeed(limit?: number): Promise<{
    success: boolean;
    posts?: Array<{
      id: string;
      title: string;
    ...
    return await window.electronAPI.invoke('molt:getFeed', { limit })
  }

  async shareToMolt(message: {
    id: string;
    text: string;
    senderId: string;
    senderName: string;
    ...
    return await window.electronAPI.invoke('molt:shareToMolt', {
      message,
      ...options
    })
  }
  ...
  // ========================================================================
  // Contact Detail Sections (Trust, Certificates, Shared)
  // ========================================================================
   ...
   */
  async getTrustAttestationsAbout(personId: string): Promise<{
    attestations: Array<{
      peer: string;
    ...
    const result = await window.electronAPI.invoke('trust:getAttestationsAbout', { personId })
    return result?.data || result || { attestations: [], error: result?.error }
  }
   ...
   */
  async getTrustAttestationsBy(personId: string): Promise<{
    attestations: Array<{
      peer: string;
      peerPublicKey: string;
      status: string;
    ...
    const result = await window.electronAPI.invoke('trust:getAttestationsBy', { personId })
    return result?.data || result || { attestations: [], error: result?.error }
  }

  /**
   * Get certificates issued FOR a person (where they are the subject).
   */
  async getCertificatesFor(personId: string): Promise<{
    certificates: Array<{
      id: string;
    ...
    const result = await window.electronAPI.invoke('trust:getCertificatesFor', { personId })
    return result?.data || result || { certificates: [], error: result?.error }
  }

  /**
   * Get certificates issued BY a person (where they are the issuer).
   */
  async getCertificatesBy(personId: string): Promise<{
    certificates: Array<{
      id: string;
      certificateType: string;
      status: string;
    ...
    const result = await window.electronAPI.invoke('trust:getCertificatesBy', { personId })
    return result?.data || result || { certificates: [], error: result?.error }
  }
   ...
   */
  async getSharedWithContact(personId: string): Promise<{
    items: Array<{
    ...
    const result = await window.electronAPI.invoke('sharing:getSharedWith', { personId })
    return result?.data || result || { items: [], error: result?.error }
  }

  /**
   * Get data that COULD be shared with a contact (not yet shared).
   */
  async getMayShareWithContact(personId: string): Promise<{
    items: Array<{
      type: 'topic' | 'profile' | 'whatsapp';
      id: string;
      name?: string;
    ...
    const result = await window.electronAPI.invoke('sharing:getMayShareWith', { personId })
    return result?.data || result || { items: [], error: result?.error }
  }
}
