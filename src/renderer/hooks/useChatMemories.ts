...
export function useChatMemories(topicId: string): UseChatMemoriesResult {
  ...
  const refreshStatus = useCallback(async () => {
    ...
    try {
      ...
      const status = await window.electronAPI?.invoke('memory:getStatus', { topicId });
  ...
  const toggle = useCallback(async () => {
    ...
    try {
      ...
      // Call IPC to toggle
      const result = await window.electronAPI?.invoke('memory:toggle', { topicId });
  ...
  const disable = useCallback(async () => {
    ...
    try {
      ...
      // Call IPC to disable
      await window.electronAPI?.invoke('memory:disable', { topicId });
...
export function useRelatedMemories(topicId: string, keywords: string[], limit = 5) {
  ...
  const loadMemories = useCallback(async () => {
    ...
    try {
      ...
      const result = await window.electronAPI?.invoke('memory:find', {
        topicId,
        keywords,
        limit
      });
  ...
  const extract = useCallback(
    async (limit = 50) => {
      ...
      try {
        ...
        const response = await window.electronAPI?.invoke('memory:extract', {
          topicId,
          limit
        });
