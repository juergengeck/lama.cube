...
import { PlansProvider, type LAMAPlansContext, type MemoryPlan, type LocalModelsPlan, type IngestionPlan, type OnecorePlan } from '@ui/core'
...
import type { JournalPlan } from '@refinio/assembly.core/plans/JournalPlan.js'
...
const journalPlan: JournalPlan = {
  ...
  async createEntry(entry: any) {
    return await window.electronAPI.invoke('journal:recordLLMCall', entry)
  },

  async queryAssemblies(options?: any) {
    const result = await window.electronAPI.invoke('journal:queryAssemblies', options || {})
...
const memoryPlan: MemoryPlan = {
  ...
  async listMemories(params) {
    return await window.electronAPI.invoke('memory:listMemories', params)
  },

  async exportMemory(params) {
    return await window.electronAPI.invoke('memory:exportMemory', params)
  },

  async importMemory(params) {
    return await window.electronAPI.invoke('memory:importMemory', params)
  },

  async previewImport(params) {
    return await window.electronAPI.invoke('memory:previewImport', params)
  }
}
...
export const electronPlans: LAMAPlans = {
  ...
  settings: {
    ...
    // API Key management - stored securely in UserSettings
    ...
    async setApiKey(provider: string, apiKey: string) {
      return await window.electronAPI.invoke('settings:setApiKey', { provider, apiKey })
    },
    async removeApiKey(provider: string) {
      return await window.electronAPI.invoke('settings:removeApiKey', { provider })
    },
    async getAllApiKeys() {
      return await window.electronAPI.invoke('settings:getAllApiKeys', {})
    }
  },
  ...
  baileys: {
    ...
    async unlink() {
      return await window.electronAPI.invoke('baileys:unlink')
    },
    async getStatus() {
    ...
    async getChats() {
      return await window.electronAPI.invoke('baileys:getChats')
    },
    async setChatPreference(params: { chatJid: string; importContacts: boolean }) {
      return await window.electronAPI.invoke('baileys:setChatPreference', params)
    },
    async setChatEnabled(params: { chatJid: string; enabled: boolean }) {
      return await window.electronAPI.invoke('baileys:setChatEnabled', params)
    },
    async confirmImport() {
      return await window.electronAPI.invoke('baileys:confirmImport')
    }
  },

  // Molt Plan (Moltbook AI social network)
  molt: {
    async getConfig() {
